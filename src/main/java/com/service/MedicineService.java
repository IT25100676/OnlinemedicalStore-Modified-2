...
public class MedicineService {
    ...
    public Medicine updateMedicine(Long id, Map<String, String> params) {
        Optional<Medicine> medicineResult = medicineRepository.findById(id);
        if (medicineResult.isEmpty()) {
            throw new RuntimeException("Medicine not found");
        }
        Medicine m = medicineResult.get();
        applyCommonFields(m, params);
    ...
    private Medicine createByType(String type) {
        String medicineType = type.toUpperCase();
        if ("TABLET".equals(medicineType)) {
            return new Tablet();
        } else if ("SYRUP".equals(medicineType)) {
            return new Syrup();
        } else if ("INJECTION".equals(medicineType)) {
            return new Injection();
        } else {
            throw new RuntimeException("Unknown medicine type: " + type);
        }
    }
